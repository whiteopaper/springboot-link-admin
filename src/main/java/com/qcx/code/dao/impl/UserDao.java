...
public class UserDao extends BaseDaoImpl implements IUserDao {
	...
	private String userPageSql(UserInfo user, int type) {
		...
		if (type == 0) {
			sql.append(" select  u.uid,u.name,u.vsername,u.password,u.mobile,u.createTime,u.state,u.deptid,d.name as deptName from t_web_user u  ");
	...
	@Override
	public UserRole select(UserRole userRelationRole) {
		List<UserRole> list = super.select(userRelationRole);
		if (list!=null){
			return  list.get(0);
		}
		return null;
